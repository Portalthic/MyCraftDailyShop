...
public final class ShopDatabase implements AutoCloseable {
    ...
    public ShopSnapshot createSnapshot(String shopId, String scopeId, String cycleKey, long expiresAt, List<Offer> generated) throws SQLException {
        ...
        try (Connection connection = dataSource.getConnection()) {
            ...
            try {
                ...
                try (PreparedStatement ps = connection.prepareStatement("INSERT INTO shop_offer(cycle_id,offer_index,provider,item_id,money,total_money,amount,personal_limit,server_limit,chance) VALUES(?,?,?,?,?,?,?,?,?,?)")) {
                    for (Offer offer : generated) {
                        ...
                        ps.setInt(8, offer.getPersonalLimit()); ps.setInt(9, offer.getServerLimit()); ps.setDouble(10, offer.getChance()); ps.addBatch();
                    }
                    ps.executeBatch();
                }
                connection.commit();
                List<Offer> offers = new ArrayList<>();
                for (Offer offer : generated) offers.add(new Offer(cycleId, offer.getIndex(), offer.getProvider(), offer.getItemId(), offer.getMoney(), offer.getTotalMoney(), offer.getAmount(), offer.getPersonalLimit(), offer.getServerLimit(), offer.getChance()));
    ...
    private List<Offer> loadOffers(Connection connection, String cycleId) throws SQLException {
        List<Offer> offers = new ArrayList<>();
        try (PreparedStatement ps = connection.prepareStatement("SELECT offer_index,provider,item_id,money,total_money,amount,personal_limit,server_limit,chance FROM shop_offer WHERE cycle_id=? ORDER BY offer_index")) {
            ps.setString(1, cycleId);
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) offers.add(new Offer(cycleId, rs.getInt(1), rs.getString(2), rs.getString(3), rs.getBigDecimal(4), rs.getBigDecimal(5), rs.getInt(6), rs.getInt(7), rs.getInt(8), rs.getDouble(9)));
            }
        }
        return offers;
    }
